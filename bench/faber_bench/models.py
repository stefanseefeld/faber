...
from PyQt6.QtGui import QStandardItemModel, QStandardItem, QBrush, QColor, QIcon, QPixmap
...
class ArtefactTreeModel(QStandardItemModel):
    ...
    def __init__(self):
        ...
        self.setHeaderData(1, Qt.Orientation.Horizontal, 'filename')
        self._module_items = {}
        self._rows = {}
        self.filter = None
    ...
    def insert(self, a):
        if not self.filter or self.filter(a):
            ...
            if a.status is not None:
                if a.isfile:
                    row[1].setData(a._filename, Qt.ItemDataRole.DisplayRole)
                color = self.green if a.status else self.red
                row[0].setData(QBrush(color), Qt.ItemDataRole.BackgroundRole)
    ...
    def update(self, artefact):
        ...
        if row:
            if artefact.status is None:  # clear
                row[0].setData(None, Qt.ItemDataRole.BackgroundRole)
                row[1].setData(None, Qt.ItemDataRole.DisplayRole)
            else:
                if artefact.isfile:
                    row[1].setData(artefact._filename, Qt.ItemDataRole.DisplayRole)
    ...
    def _make_row(item):
        ...
        row[0].setData(item.name, Qt.ItemDataRole.DisplayRole)
