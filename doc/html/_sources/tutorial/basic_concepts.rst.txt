...
As we have seen in the previous section, a `fabscript` defines :term:`artefact`\ s,
mostly by calling :term:`rule`\ s that express how artefacts can be generated by
invoking :term:`action`\ s. In this chapter we will look a little closer at how to use them.
