...
def run(builddir, srcdir, rc):
    ...
    with project(info) as proj:
        jobs = int(info._config['bench']['parallelism']) if 'parallelism' in info._config['bench'] else 1
        timeout = int(info._config['bench']['timeout']) if 'timeout' in info._config['bench'] else 0
        recipe.recipe.init(jobs=jobs, timeout=timeout)
