...
class map(object):
    ...
    def __init__(self, feature, func=None, *args, **kwds):
        ...
        if func is None: func = lambda fv: fv.value if fv is not None else []
        def wrapper(fs):
            fv = next(iter(filter(lambda v:v._type is feature, fs._features.values())), None)
...
def translate(fv, prefix='', suffix=''):
    ...
    return ['{}{}{}'.format(prefix, v, suffix) for v in fv.value] if fv else []

def join(fv, char=os.pathsep):

    return [char.join([v for v in fv.value] if fv else [])]

def select_if(fv, ref, value):
    """Return a given value if a given feature has a specific value"""
    
    return [value] if fv and fv.value == ref else []
